...
namespace mypo
{
    ...
    public class PgnUtil {
        ...
        /// </returns>
        public static StreamReader OpenInpFile(string strInpFileName) {
            StreamReader  streamInp;
            
            try {
                streamInp = new StreamReader(strInpFileName, Encoding.GetEncoding(1251));
            } catch(System.Exception) {
                Console.WriteLine("Unable to open the file - " + strInpFileName);
                streamInp = null;
            }
            return(streamInp);
        }
        ...
        public static Stream CreateOutFile(string strOutFileName) {
            ...
            try {
                ...
                Console.WriteLine("Unable to create the file - " + strOutFileName);
                streamOut = null;
            }
            return(streamOut);
        }
        ...
        private int ScanPGN(Stream streamInp, Dictionary<String, String> hashPlayerList, ref int iMinELO, ref int iMaxELO) {
            ...
            try {
                ...
                Console.WriteLine("Error around the line " + m_iCurLine.ToString() + " while parsing the PGN file.\r\n" + exc.Message);
                iRetVal = -1;
            }
            return(iRetVal);
        }
 ...
 */
        ...
        /// Number of games found in the stream or -1 if error.
        /// </returns>
        public List<vanparty> FillListBoxWithDesc(StreamReader streamInp) {
            List<vanparty> lRet;
            List<String>    arrPGNGame;
            int             iGameStartPos;
            int             iDummy;
            ...
            m_iCurLine      = 0;
            m_strPushedLine = null;
            lRet         =  new List<vanparty>(100);
            try {
                arrPGNGame = GetPGNGame(reader, out iGameStartPos);
                while (arrPGNGame != null) {
                    GetPGNGameInfo(arrPGNGame, iGameStartPos, out iDummy, out iDummy);
                    strDesc = GetPGNGameDesc(arrPGNGame, iGameStartPos);
                    lRet.Add(new vanparty(strDesc, arrPGNGame, iGameStartPos));
